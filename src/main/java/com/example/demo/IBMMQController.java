	...
	public void sendMessage(@RequestBody Message ibmmqmessage)
	{
		Connection connection = null;
		 ...
		 try {
				  ...
				  connection = cf.createConnection(); session = connection.createSession(false,Session.AUTO_ACKNOWLEDGE); destination = session.createQueue("queue:///servicein");
				  
				  producer = session.createProducer(destination); TextMessage message =
				  session.createTextMessage(ibmmqmessage.getData()); connection.start();
